...
export default function DashboardESamsatMain() {
    ...
    async function onSubmit(data: FormValues) {
        ...
        try {
            const response = await fetch('http://10.30.1.11:8000/online/searchRanmor', {
                method: 'POST',
                headers: {
    ...
    async function onConfirm() {
        ...
        try {
            ...
            const response = await fetch('http://10.30.1.11:8000/online/tap', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
    ...
    return (
        <div className="flex flex-col size-full items-center justify-center p-6">
            ...
            <div className="w-full max-w-6xl">
                ...
                {informationData && (
                    <div className="flex justify-center items-center p-8">
                        <p className='flex text-center'>{informationData.m}</p>
                    </div>
                )}
            </div>
        </div>
